	...
	public class MainClass {
...
#if NETCOREAPP
			result.Add(Path.Combine(sourceDirectory, AppDomain.CurrentDomain.FriendlyName + ".exe"));
			result.Add(Path.Combine(sourceDirectory, AppDomain.CurrentDomain.FriendlyName + ".dll"));
#else
		...
		private static void UpdateSubDirectories(string sourceDirectory, string destinationDirectory) {
			...
			foreach(string sourceSubDirectory in sourceSubDirectories) {
				...
				string destinationSubDirectory = destinationDirectory + new DirectoryInfo(sourceSubDirectory).Name;  // custom code
